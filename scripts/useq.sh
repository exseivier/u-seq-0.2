		...
		-)
			case $OPTARG in
				query)
					...
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Query genome path: $PATH_QUERY_GENOME"
				;;
				db)
					PATH_GENOMES_DB=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Genomes database path: $PATH_GENOMES_DB"
				;;
				size)
					SIZE=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Window size: $SIZE"
				;;
				steps)
					STEPS=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Size of the step: $STEPS"
				;;
				task)
					TASK=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Required task: $TASK"
				;;
				qcov)
					QCOV=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Query coverage percentage: $QCOV"
				;;
				identity)
					IDENTITY=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Identity percentage: $IDENTITY"
				;;
				evalue)
					EVALUE=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - e-value cutoff: $EVALUE"
				;;
				gopen)
					GOPEN=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Gap opening cost: $GOPEN"
				;;
				gext)
					GEXT=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Gap extension cost: $GEXT"
				;;
				reward)
					REW=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Matching reward: $REW"
				;;
				penalty)
					PEN=${!OPTIND}
					OPTIND=$(( $OPTIND + 1 ))
					echo "[PARSING ARGS!] - Mismatching penalty: $PEN"
				;;
				help)
					echo "[USAGE!] - useq --arguments --etc"
					exit 1
				;;
				?)
